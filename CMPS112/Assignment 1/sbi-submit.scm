      ...
      ((pair? expr)   
         (begin
            (if (hash-has-key? *function-table* (car expr))
               ...
               ; Else
               (err-syntax #:msg (string-append "Command " 
                  (car expr) " not found."))
            )
         )
      )
      (else 
         (begin
            (if (hash-has-key? *variable-table* expr)
               (variable-get expr)
               ; Else
               ;(err-syntax #:msg (string-append "Variable " 
               ;   (symbol->string expr) " not found."))
               0 ; Since un-init variables are supposed to be 0
            )
         )
      )
   )
...
(define (basic-print msg)
   (if (null? msg) ; Handle (print)
      (newline)
         ; Make sure there's actually something to do
      (when (not (null? (car msg))) 
         ...
         (if (not (null? (cdr msg))) ; There's still things to print
            (basic-print (cdr msg))
            ; We're out of things to print, newline to end the print
            (newline)
         )
      )
   )
...
(define (basic-dim param)
   (when (not (null? param))
      (let ((pa (car param)))
         (variable-put! (car pa) 
            (make-vector (+ (evalexpr (cadr pa)) 1) 0))
         (function-put! (car pa) (lambda (x) 
            (vector-ref (variable-get (car pa)) x)))
      )
   )
)
...
(define (basic-let param)
   (when (not (null? param))
      (if (pair? (car param))
         ; The param is a pair, accessing an index of an array
         ; Set the vector at index to the value
         (vector-set! (variable-get (caar param)) (exact-floor 
            (evalexpr (cadar param))) (evalexpr (cadr param)))
...
; Can have n number of parameters, and each time enter is pressed that 
;  variable gets set and the function moves on to the next one
      ...
      (let ((in (read)))         
         (if (eof-object? in) ; If CRTL-D
            ...
            (if (number? in) ; Make sure the input is numeric
               ; Now assign the variable to the input, recuse if needed
               (variable-put! (car param) in)
               
               ; Else, number isn't numeric, put nan into the variable
               (variable-put! (car param) +nan.0)
            )
               ; Add 1 to count and recurse if there's more variables
            (if (not (null? (cdr param)))
               (basic-input (cdr param) #:cnt (+ 1 count))
               ; If not, just put the count into inputcount
               (variable-put! 'inputcount count) 
            )
         ))
      )      
   )
...
; Initialize all the functions we allow 
; the BASIC program to have, at the start
(for-each
   (lambda (pair)
   ...
   `(
      ...
      (exp    ,exp)
      (floor   ,floor)
      ; Return -inf.0 if log of 0
      (log    ,(lambda (x) (if (eq? x 0) (/ (- 1) (+ x 0.0)) (log x))))
      (sqrt   ,sqrt)
      (atan    ,atan)
      (sin    ,sin)
      (cos    ,cos)
...
(define (readlist-from-inputfile filename)
   ; Looping through the file? Or maybe it reads it in one read
   (let ((inputfile (open-input-file filename)))
       ; Make sure it's able to be opened
...
(define (eval-labels program)
   (when (not (null? program))
      ; Get the first current line of the program
      (let ((1st (first program)))
         ; Make sure there are things in the cdr
         (when (not (null? (cdr 1st)))
...
(define (exec-line command program line-num)
   (if (hash-has-key? *function-table* (car command))
      (begin ; We need some special cases for some commands
         (cond 
            ; Goto needs the entire program to be able to pass it back
            ;   into run-program with a different line number
            [(eqv? 'goto (car command))
               ; No need to go through the function-get
               (basic-goto (cadr command) program)] 
            ; If needs the program so it can call goto if needed
            [(eqv? 'if (car command)) 
...
(define (run-program program line-num)
   ; Make sure we haven't run out of program to execute
   (when (>= (length program) line-num)
      
      ; Get the element at line-num and parse it
      (let ((line (list-ref program (- line-num 1))))
         ; Parse and run the line
         ; The only thing that matters is the statement
         
         (when (> (length line) 1) ; Check for just a line number

            (begin ; There is more than just a line number
          ; If the line has 2 parts, means there's 2 options for format
               (when (equal? (length line) 2)

                  (when (pair? (second line)) ; If line # + cmd
                     (exec-line (second line) 
                         program line-num) ; Execute line

                     ; Else, line # + label, so ignore
                  )
               )
               ; The line has 3 parts, which is line # + label + command
               (when (equal? (length line) 3)
